...
lastCar.info = function(){
    return `The last car is a ${this.car_year} ${this.car_make} ${this.car_model}`;
}

console.log(lastCar.info());
...
function sortArray(item, arr, key) {
    for (let i = 0; i < arr.length; i++){
        item.push(arr[i][key]);
    }
    return item;
...
sortArray(carModels, inventory, "car_model");
...
function getCarYear(arr, key) {
    for ( let i = 0; i < arr.length; i++){
        carYears.push(arr[i][key]);
    }
    return carYears;
}
carYears = getCarYear(inventory, "car_year");
